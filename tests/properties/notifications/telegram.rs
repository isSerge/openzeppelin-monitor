//! Property-based tests for Telegram notifications.
//!
//! These tests verify the behavior of the Telegram notification system using property testing,
...
proptest! {
	...
	fn test_notification_template_idempotency(
		...
		vars in template_variables_strategy()
			...
			Some("https://telegram.com/test".to_string()),
			"".to_string(),
			"".to_string(),
			None,
			"Test".to_string(),
	...
	/// - The formatted message follows the expected Telegram format: "*Title* \n\ntemplate"
	#[test]
	fn test_notification_empty_variables(
		...
		let escaped = TelegramNotifier::escape_markdown_v2(&template);
